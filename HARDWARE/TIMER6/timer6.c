...
void TIM6_Init(void)
{
	...
	TIM_TimeBaseInit(TIM6,&TIM_TimeBaseInitStructure);            //设置TIM2
	
	TIM_ClearITPendingBit(TIM6,TIM_IT_Update);                    //清除溢出中断标志位
	TIM_ITConfig(TIM6,TIM_IT_Update,ENABLE);                      //使能TIM2溢出中断    
	TIM_Cmd(TIM6,ENABLE);                                         //开TIM2                          
	
	NVIC_InitStructure.NVIC_IRQChannel=TIM6_IRQn;                 //设置TIM2中断
